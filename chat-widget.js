...
let chat = {
    init(colorSettings, domain='http://95.216.97.37:8777'){
      this.config = {
        domain,
      }
    ...
    insertChat(colorSettings){
      ...
      .chat-bubble{
        ...
        cursor: pointer;
        background-color: ${colorSettings.chatColor};
      }
      .chat-bubble svg{
        width: 40px;
        height: 40px;
      ...
      .chat-submit{
        ...
        width: 36px;
        height: 36px;
        background-color: ${colorSettings.messageColor};
        border: none;
        align-self: flex-end;
      }
    ...
    async onUserRequest(message){
      try{
        ...
        const response = await this.sendRequestToBot(this.config.domain, message)
        ...
        switch(e.message){
          case '422': this.reply('Error 422: app_id неверный');
            break;
          case '403': this.reply('Error 403: api_hash неверный');
            break;  
          default: this.reply('Error');
            break;
        }
      }
    },
    ...
    async sendRequestToBot(domain, message){
      return await fetch(`${domain}/api/v1/webhook/chat/handle_message`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          app_id: "018d20bd-2bc7-8392-aa57-e937ec9e7757",
          api_hash: "3IsJeVI3Op0bsjTS8AeCeJ3b3PHhyUz7",
          message
        })
      })
    }
  ...
  chat.init(
    {
      chatText: 'Майа', 
      chatColor: 'rgb(56, 61, 69)', 
      messageColor: 'rgb(24, 193, 57)'
    }
  )